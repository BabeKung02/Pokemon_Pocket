...
const Sub_Header = ({ handleSearchChange, setSearchPoke }) => {
    const [isFocused, setIsFocused] = useState(false);
    const [showClearIcon, setShowClearIcon] = useState(false);
    const { addToPocket , pocketCount} = usePocket();
    
    const handleFocus = () => {
        setIsFocused(true);
        setShowClearIcon(true); // แสดงไอคอนลบเมื่อโฟกัส
    };

    const handleBlur = (event) => {
        setIsFocused(false);
        if (!event.target.value) {
            setShowClearIcon(false); // ซ่อนไอคอนลบเมื่อไม่มีข้อมูลใน input หลังจาก blur
        }
    };
            ...
            <div className="user-controls d-flex align-items-center">
                ...
                <div className="pocket position-relative">
                    ...
                    <div className="position-relative ms-3">
                        <Link to="/pocket" className="text-dark text-decoration-none">
                            <span
                                id="pocket_count"
