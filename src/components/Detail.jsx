...
const Detail = () => {
    ...
    const { addToPocketHandler, addToPocket, allData, setAllData } = useContext(PocketContext);
    ...
    const handleClickToAdd = () => {
        ...
        // ตรวจสอบว่า id นี้มีอยู่ใน allData หรือไม่
        const existingIndex = allData.findIndex(item => item.id === id);

        if (existingIndex !== -1) {
            // ถ้ามีอยู่แล้ว ทำการอัปเดต quality และ totalQuantity ของข้อมูลที่มีอยู่
            const updatedData = [...allData];
            updatedData[existingIndex] = {
                ...updatedData[existingIndex],
                quantity: updatedData[existingIndex].quantity + quantity,
                totalQuantity: updatedData[existingIndex].totalQuantity + quantity,
            };
            setAllData(updatedData);
        } else {
            // ถ้ายังไม่มีใน allData ให้เพิ่มข้อมูลใหม่เข้าไป
            ...
            {pokemon && (
                <div className="row align-items-center d-flex">
                    ...
                    <div className="col-md-6">
                        ...
                        <button onClick={handleClickToAdd} style={{ marginRight: '10px', backgroundColor: '#FF6F61', color: 'white', padding: '15px 60px', borderRadius: '10px' }} className="btn btn-block align-items-center d-flex">
